...
// Подсказки для каждой роли
const ROLE_HINTS = {
    applicant: {
        name: "Гость (абитуриент)", icon: "🎒",
...
// === ШАПКА: бейдж и кнопка ===
function updateHeaderBadge() {
    const r = ROLE_HINTS[currentUser.role] || ROLE_HINTS.applicant;
    document.getElementById("roleBadge").textContent = `${r.icon} ${r.name}`;
    document.getElementById("loginBtn").textContent =
        currentUser.auth ? "🚪 Выйти" : "🔑 Войти";
}

// === МОДАЛКА ВХОДА ===
function openLogin() {
    // Если уже авторизован — выход
    if (currentUser.auth) {
...
function doLogin() {
    ...
    if (!acc || acc.password !== pass) {
        err.textContent = "⚠️ Неверный логин или пароль.";
        return;
    }

    currentUser = { role: acc.role, name: acc.name, login, auth: true };
    localStorage.setItem(USER_KEY, JSON.stringify(currentUser));
    closeLogin();
    updateHeaderBadge();
    renderWelcome();
...
function useSuggestion(el) { document.getElementById("userInput").value = el.textContent; sendMessage(); }
...
function renderWelcome() {
    ...
    document.getElementById("chatMessages").innerHTML = `
        <div class="welcome-message">
            <div class="welcome-icon">${r.icon}</div>
            <h3>Здравствуйте!</h3>
            <p>${r.description}</p>
...
function addMessageToDOM(msg) {
    ...
    if (msg.role === "bot") {
        if (msg.error) {
            contentHTML = `<div class="error-message">⚠️ ${escapeHtml(msg.error)}</div>`;
        } else {
            if (msg.sql) {
                contentHTML += `
                <div class="sql-block">
                    <div class="sql-header"><span>SQL-запрос</span>
                    <button onclick="copySQL(this)">📋 Копировать</button></div>
                    <pre><code class="language-sql">${escapeHtml(msg.sql)}</code></pre>
                </div>`;
            }
            if (msg.data && msg.data.length > 0) {
                contentHTML += renderTable(msg.data);
                contentHTML += `<button class="csv-btn" onclick="exportCSV(this)">⬇️ Экспорт в CSV</button>`;
            } else if (msg.sql) {
                contentHTML += `<div style="margin-top:8px;color:#64748b;font-style:italic;">✓ Запрос выполнен. Данные не найдены.</div>`;
            }
        }
    }
